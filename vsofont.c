...
struct vsofont *examplefont, *clone, *proportional;
int font_init()
{

    glEnableClientState(GL_VERTEX_ARRAY);
/* load the font */
if (vsofont_read_font_from_file("example.vsofont", &examplefont) == FAILED)
  return 1;
else
    printf("font loaded\n");
/* alter it */
vsofont_recreate_vertices(examplefont, 0.1, 0.1);
vsofont_set_spacing(examplefont, 2.35);

/* clone it */
vsofont_clone(examplefont, &clone);

/* make the clone look different */
vsofont_set_color(clone, 0.0, 0.5, 1.0, 1.0);
vsofont_set_spacing(clone, 0.02);

/* clone again */
vsofont_clone(clone, &proportional);

/* make the clone look different */
vsofont_set_type(proportional, VSOFONT_TYPE_PROPORTIONAL);
vsofont_set_color(proportional, 0.7, 0.5, 1.0, 1.0);
vsofont_set_spacing(proportional, 0.023);

vsofont_recreate_vertices(clone, 0.05, 0.05);
}
...
static int _vsofont_write_string(struct vsofont *font, unsigned char *u) {
  ...
  glVertexPointer(2, GL_FLOAT, 0, font->vertices);
  glColor4f(font->r, font->g, font->b, font->a);
...
int vsofont_character_create(struct vsocharacter **c, int i) {
  ...
  *c = malloc(sizeof(struct vsocharacter));
  if (*c == NULL) {
  ...
  if (i < 256)
    ...
    (*c)->indices = malloc(i*sizeof(unsigned short));
  if ((*c)->indices == NULL) {
    free(*c);
    *c = NULL;
    fprintf(stderr, "VSOFONT_CHARACTER_CREATE: Out of memory error.\n");
...
int vsofont_create(struct vsofont **font, int x, int y, float sx, float sy, float r, float g, float b, float a, float spacing) {
  ...
  *font = NULL;
  ...
  f = malloc(sizeof(struct vsofont));
  if (f == NULL) {
  ...
  f->vertices = malloc(sizeof(float) * x*y * 2);
  if (f->vertices == NULL) {
    free(f);
    fprintf(stderr, "VSOFONT_CREATE: Out of memory error.\n");
    return FAILED;
  ...
  f->characters = malloc(sizeof(struct vsocharacter *) * 256);
  ...
  if (x*y < 255)
    f->indices_type = GL_UNSIGNED_BYTE;
  else
    f->indices_type = GL_UNSIGNED_SHORT;
...
int vsofont_clone(struct vsofont *font_in, struct vsofont **font_out) {
  ...
  *font_out = NULL;
  ...
  /* create list of grid's vertices */
  f->vertices = malloc(sizeof(float) * font_in->grid_dx*font_in->grid_dy * 2);
  if (f->vertices == NULL) {
    free(f);
    fprintf(stderr, "VSOFONT_CLONE: Out of memory error.\n");
    return FAILED;
